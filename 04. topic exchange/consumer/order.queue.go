...
func orderQueue() {
	...
	q, err := ch.QueueDeclare("shop_order_queue", true, false, false, false, amqp091.Table{
		"x-queue-type": "quorum",
	})
	if err != nil {
	...
	err = ch.QueueBind(q.Name, "order.#", "x.shop.events", false, nil)
	if err != nil {
	...
	for msg := range msgs {
		log.Println(string(msg.Body), " : from shop order queue")
		msg.Ack(false)
	}
}
